...
|---|---|---|---|---|---|
| 1 | Status, Checking… | `status-checking.png` | Hero ring sweeps with `ringSweep`; native pull-to-refresh owns manual refresh progress | `Checking…` (existing) | RD-4, RD-5 |
...
| 7 | Reduce Transparency, Status | `status-reduce-transparency.png` | The native tab bar uses its system fallback appearance; Status has no separate refresh button | — | RD-4, RD-5, RD-12 |
...
| 9 | Live Activity, Dynamic Island, Home Screen widget: stale and checking | `widgets-live-activity-stale.png`, `widgets-live-activity-checking.png` | Stale: clock symbol in light grey `statusNoData` with a plain title, "No Current Data" (REQ-SURF-010, amended 2026-09-23; it was `statusStale` yellow until then), time with "last known", footer "Last known: {status}". Checking: spinner, `statusChecking` | Stale title per casing rule; the key named `liveActivity.stale` reads "Updating…" and is used for the **idle** phase — no data yet — while real staleness uses `liveActivity.staleFooter` ("Last known: %@"). RD-11 verified on a533bb6 that the "Updating… shown for stale data" warning this row used to carry is **not reproducible**: the behaviour is correct and only the key's name is wrong. The rename is tracked separately; do not "fix" the idle path. **A stale alarm does not downgrade**: it keeps "Alert" in `statusAlert`, because a driver reading a red-to-neutral change as "no alert" is the one failure this app must never produce (`docs/core.md` priorities: honest signal). Only quiet downgrades — a stale "No Alert" loses its clear color, since a stale clear signal is the dangerous one. The mockup shows the quiet case only because it is the one that changes (drivecheck-product, 2026-09-17, confirming RD-10's reading) | RD-10 |
