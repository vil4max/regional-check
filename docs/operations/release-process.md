...
6. A release tag may be moved only while no build of that version was submitted to App Review or released, only by the owner, and only to a later commit on `main` that passes the same checks. After submission the tag is never moved or reused; fix a bad release with a new patch version. `v3.0.0` predates ADR 0013 and still marks a candidate build that was never submitted, so it may still be moved onto the commit that is eventually submitted.
